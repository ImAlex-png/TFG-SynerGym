...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <h1 class="text-3xl font-black italic tracking-tighter text-white uppercase">Mis Entrenamientos</h1>
          ...
          @for (insc of inscripciones(); track insc.id) {
            <div class="bg-bg-card p-6 rounded-2xl border border-white/10 flex justify-between items-center group hover:border-primary/50 transition-all">
              <div class="flex items-center space-x-6">
                <div class="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center text-primary font-bold">
                  {{ insc.clases.nombre.charAt(0) }}
                </div>
                <div>
                  <h3 class="font-bold text-white text-lg">{{ insc.clases.nombre }}</h3>
                  <p class="text-sm text-gray-500">
                    {{ insc.clases.fechaInicio | date }} • {{ insc.clases.horaInicio.slice(0,5) }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-center space-x-8">
                <div class="text-right">
                  ...
                  <p class="text-[10px] text-gray-500 mt-1 uppercase tracking-tighter font-bold">
                    {{ insc.pagado ? 'PAGADO' : 'PENDIENTE PAGO' }}
                  </p>
                </div>
                <button 
                  (click)="cancelar(insc.id)"
