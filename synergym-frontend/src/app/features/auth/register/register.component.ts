...
@Component({
  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-bg-dark relative overflow-hidden px-4">
      ...
      <div class="bg-bg-card p-10 rounded-[40px] border border-white/10 shadow-2xl w-full max-w-lg backdrop-blur-md z-10">
        <div class="text-center mb-10">
          <h1 class="text-4xl font-black italic tracking-tighter text-white uppercase">
            JOIN <span class="text-primary">SYNERGYM</span>
          </h1>
          <p class="text-gray-400 mt-2 font-medium">Crea tu cuenta y empieza a entrenar.</p>
        </div>
        ...
        <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" formControlName="nombre" class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary transition-all">
            </div>
            <div>
              <label class="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-[0.2em]">Apellidos</label>
              <input type="text" formControlName="apellidos" class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary transition-all">
            </div>
          </div>

          <div>
            <label class="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-[0.2em]">Email</label>
            <input type="email" formControlName="email" class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary transition-all">
          </div>

          <div>
            <label class="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-[0.2em]">Teléfono</label>
            <input type="text" formControlName="telefono" class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary transition-all">
          </div>

          <div>
            <label class="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-[0.2em]">Contraseña</label>
            <input type="password" formControlName="password" class="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-primary transition-all">
          </div>

          @if (error()) {
            <div class="bg-red-500/10 border border-red-500/20 text-red-500 p-4 rounded-2xl text-xs text-center font-bold">
              ⚠️ {{ error() }}
            </div>
          }

          <button type="submit" [disabled]="loading()" class="w-full bg-primary hover:bg-primary/90 text-white font-black py-5 rounded-2xl transition-all shadow-lg shadow-primary/20 uppercase tracking-[0.1em] active:scale-95">
            {{ loading() ? 'PREPARANDO TU PERFIL...' : 'ENTRAR AL GYM AHORA' }}
          </button>
        </form>

        <div class="mt-8 text-center text-xs text-gray-500 font-medium">
          ¿Ya eres atleta? <a routerLink="/login" class="text-secondary font-black hover:underline uppercase ml-1">Inicia sesión</a>
        </div>
      </div>
    </div>
  `
